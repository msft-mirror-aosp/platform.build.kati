...
void Flags::Parse(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (ParseCommandLineOptionWithArg("--variable_assignment_trace_filter",
                                             argv, &i,
  ...
  if (traced_variables_pattern.size() && dump_variable_assignment_trace == nullptr) {
    ERROR("--variable_assignment_trace_filter is valid only together with --dump_variable_assignment_trace");
  }
}
